...
class Search extends React.Component<any, any>  {
  ...
  getOperator(cn: any, idx: any) {
    ...
    if(cn.operator) {
      return (
        <select key={key_prefix} value={cn.operator} onChange={(key_prefix) => this.handleChange(key_prefix)}>
  ...
  getConditionGroup(cn: any, idx: any) {
    ...
    cn.operand.map((c: any, _id: any) => {
      if(c.operator) {
        cn_dom.push(
          <select key={key_prefix + 'o' +_id} value={c.operator} onChange={(key_prefix) => this.handleChange(key_prefix)}>
            <option value='AND'>AND</option>
            <option value='OR'>OR</option>
            <option value='NOT'>NOT</option>
          </select>
        );
        cn_dom.push(<input key={key_prefix + 'f' +_id} type="text" value={c.operand} onChange={() => this.handleChange(key_prefix)} />);
      } else {
        cn_dom.push(<input key={key_prefix + 'f' +_id} type="text" value={c.operand} onChange={() => this.handleChange(key_prefix)} />)
      }
